...
**Nova ocorrência (2026-07-06, pós-Fase 7, teste de statusLine em background)**: `fast-context` rodado em background com pergunta ampla ("descreva com calma, passo a passo, o fluxo completo... citando cada arquivo/trecho relevante") declarou "Turno 8/8" e **não emitiu `<final_answer>`** — parou no meio de uma frase de raciocínio ("vou fazer uma leitura final..."). A Camada 1 (auto-contagem) falhou de novo, mesma classe do achado da rodada 2 (stall silencioso), mas dessa vez o modelo reconheceu corretamente que chegou no turno 8 e ainda assim não obedeceu a instrução de emitir o bloco imediatamente. A Camada 2 (`limit_turns_hook.py`) não pegou esse caso porque o limite é de 10 tool calls, não 8 turnos — o subagent ficou dentro do teto de tool calls mas estourou a contagem de turnos de raciocínio. Não corrigido nesta sessão (adiado por decisão do usuário) — próximo passo natural seria alinhar os dois limites (turnos vs. tool calls) ou reforçar a Camada 2 pra cobrir turnos de raciocínio sem tool call.
