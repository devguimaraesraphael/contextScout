...
**Nova ocorrência (2026-07-06, pós-Fase 7, teste de statusLine em background)**: `fast-context` rodado em background com pergunta ampla ("descreva com calma, passo a passo, o fluxo completo... citando cada arquivo/trecho relevante") declarou "Turno 8/8" e **não emitiu `<final_answer>`** — parou no meio de uma frase de raciocínio ("vou fazer uma leitura final..."). A Camada 1 (auto-contagem) falhou de novo, mesma classe do achado da rodada 2 (stall silencioso), mas dessa vez o modelo reconheceu corretamente que chegou no turno 8 e ainda assim não obedeceu a instrução de emitir o bloco imediatamente. A Camada 2 (`limit_turns_hook.py`) não pegou esse caso porque o limite é de 10 tool calls, não 8 turnos — o subagent ficou dentro do teto de tool calls mas estourou a contagem de turnos de raciocínio.

**Duas tentativas de correção via system prompt, ambas falharam (mesma sessão, mesma pergunta reproduzida 3x)**: (1) reforço pedindo pra nunca anunciar próxima ação sem executá-la na mesma resposta — reteste falhou de novo, ainda mais cedo (turno 6/8 em vez de 8/8); (2) reforço nomeando o mecanismo real ("cada resposta precisa ter tool call OU `<final_answer>`, nunca nenhum dos dois, porque não existe próxima resposta") — reteste falhou pela terceira vez (turno 8/8 de novo). **Conclusão**: 3/3 reproduções com essa pergunta ampla e perfeccionista ("descreva com calma... citando cada arquivo relevante"); não é um problema de redação de instrução — é uma tendência estrutural do modelo sob esse tipo de prompt aberto, sem hook nativo disponível pra forçar continuação quando a resposta não tem tool call (só existe `PreToolUse`, que não dispara se não há tool call nenhum). **Mitigação movida pra downstream**: `.claude/rules/exploration.md` agora trata "sem `<final_answer>` nenhum" como equivalente a `confidence="low"` pra fins de escalonamento, e recomenda reformular a pergunta pra algo mais estreito antes de reenviar — não insistir na mesma pergunta ampla via `fast-context-deep`. Risco permanece **não mitigado na origem** (o subagent ainda pode estourar assim), só contido no consumo da resposta.
