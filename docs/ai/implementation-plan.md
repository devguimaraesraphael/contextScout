...
- [x] Calibração ao longo do tempo: feita com o gabarito acumulado da Fase 7 — achado de calibração errada em Q3 (`confidence="high"` numa resposta sobre o repositório errado) motivou a instrução de confirmação de path por `Glob` adicionada aos dois agents (ver ação de acompanhamento em `baseline-results-2026-07-06.md`, validada com re-teste). Não altera o critério de escalonamento em si (continua `confidence != "high"` ou `files_found` baixo) — a mitigação real ficou no system prompt do subagent, não na regra de escalonamento.
...
**Rodada 5 (pós-Fase 7) — mitigação do achado #3 implementada e validada**: adicionada instrução explícita de confirmação de path via `Glob` no system prompt de `fast-context.md`/`fast-context-deep.md`. Re-teste de Q3 com a mesma pergunta: `confidence="high"`, citação correta no repositório certo (`fastContextMicrosoft/src/fastcontext/agent/utils.py:90-100`), sem confundir com este projeto. Detalhe em `baseline-results-2026-07-06.md`. N=1 reteste — mitigação registrada, não garantia estatística.
